...
1. Copy `_TEMPLATE.html` to a new file in this folder, e.g.
   `25-fake-gift-card.html`. Numbering the file is a convenience for
...
2. Write the email: subject, from/to/date, and body, following the
   pattern already in the template and in `08-fedex-shipping-confirmation.html`.
...
- An `annotation-note`'s explanatory text can occasionally link to a
  genuine, real, safe external resource — e.g. pointing someone toward
  identitytheft.gov if an example discusses sensitive personal
  information. This is different from every other link on this site,
  which is a fake, inert stand-in for something malicious. A real link
  needs `class="real-link"` and an actual `href`, and should open in a
  new tab so it doesn't navigate away from the example:

  ```html
  <a href="https://www.identitytheft.gov/" class="real-link" target="_blank" rel="noopener noreferrer">IdentityTheft.gov</a>
  ```

  `real-link` opts it out of the fake-url hover tooltip (it has no
  `data-fake-url`, so without this the tooltip would still generate an
  empty box on hover). Use this rarely and deliberately — almost every
  link in this folder should be fake, and a real one should only
  appear where it's genuinely useful, safe, and well short of anything
  resembling the example's own scam content.
...
1. **The viewer loads every example in a sandboxed `<iframe>`**
   ...
   deliberately *without* `allow-scripts`, `allow-forms`, or
   `allow-top-navigation`). This is the primary control — it's enforced
   by the browser and doesn't depend on anything in this folder being
   written correctly. `allow-popups` is the one addition beyond the
   bare minimum, needed so a genuine external link (see the `real-link`
   convention below) can open in a new tab with `target="_blank"` —
   without it, the sandbox silently blocks *any* new-tab/popup
   behavior, even from a link with a real, safe `href`. This doesn't
   weaken anything for the simulated phishing content itself: every
   fake link still has no `href` at all (see below), so there's
   nothing for `allow-popups` to act on there regardless.
