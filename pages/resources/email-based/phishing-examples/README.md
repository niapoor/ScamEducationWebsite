...
- If the flagged text is immediately followed by trailing punctuation
  ...
  sender's email address — put that punctuation **inside** the `mark`,
  right after the flagged text and before the `annotation-note` span,
  rather than after the closing `</mark>`:
  ...
  stranded on its own line is the visible symptom. Moving it inside the
  `mark` keeps it glued to the flagged text on the original line; it
  picks up the same highlight and dotted underline as the rest of the
  flagged text, which is a non-issue for a single punctuation character.
