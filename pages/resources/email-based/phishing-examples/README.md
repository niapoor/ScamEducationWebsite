...
- If the flagged element already has its own strong visual treatment
  ...
  clearly marking it. Add `no-highlight` alongside `annotation`:

  ```html
  <mark class="annotation no-highlight" data-color="3"><span>Lowe's</span></mark><span class="annotation-note" data-color="3">...</span>
  ```

  This keeps the numbered badge (a separate rule, unaffected) while
  removing the background, color override, and underline from the
  flagged text itself — use it sparingly, only when the highlight
  genuinely gets in the way, not as a default.
